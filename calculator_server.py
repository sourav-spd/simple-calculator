"""calculator_server.py - Simple Calculator MCP Server with stdio, SSE and Streamable HTTP."""
...
from tools.calculator_tools import (
    ...
    AddToolHandler,
    DivideToolHandler,
    ExpressionEvaluatorToolHandler,
    FactorialToolHandler,
    ModuloToolHandler,
    MultiplyToolHandler,
    PercentageToolHandler,
    PowerToolHandler,
    RoundToolHandler,
    SquareRootToolHandler,
    SubtractToolHandler,
)
...
logger = logging.getLogger("simple-calculator-mcp")
...
def register_all_tools() -> None:
    ...
    add_tool_handler(AddToolHandler())
    add_tool_handler(SubtractToolHandler())
    add_tool_handler(MultiplyToolHandler())
    add_tool_handler(DivideToolHandler())

    # Advanced operations
    add_tool_handler(PowerToolHandler())
    add_tool_handler(SquareRootToolHandler())
    add_tool_handler(ModuloToolHandler())

    # Utility operations
    add_tool_handler(AbsoluteToolHandler())
    add_tool_handler(RoundToolHandler())
    add_tool_handler(FactorialToolHandler())
    add_tool_handler(PercentageToolHandler())

    # Expression evaluator
    add_tool_handler(ExpressionEvaluatorToolHandler())
...
async def run_server(mode: str = "stdio", host: str = "0.0.0.0", port: int = 8009) -> None:
...
def cli_main() -> None:
    """CLI entry point for the calculator-mcp command."""
    try:
        asyncio.run(main())
    except KeyboardInterrupt:
        pass
